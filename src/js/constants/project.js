export const PROJECTS = [
    {
        ...
        <p><strong>How It Works</strong>: For the homepage, the web app displays a “create an event” button and all the events the user has created. If available, the top will always showcase the latest upcoming event, the middle section will always showcase the possible future events, and the last section would showcase the past events. In each event card, users can see the following information: event name, datetime, location, edit button, delete button, and “see more” button.</p>
        <p>If the user pressed “See more”, they will not only see the core information about the event (as mentioned above) but also additional information such as the attendees and the needed items for the event. Each users has there own unique color. For the item section, each item is color coded and each item’s color corresponds to the attendees color, displaying which users are responsible for what items. In addition, users can reassign item responsibility by pressing the “Reassign Items” located at the bottom of the event page.</p>
